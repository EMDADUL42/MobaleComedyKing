...
public class MyHorizontalAdapter extends RecyclerView.Adapter<MyHorizontalAdapter.ViewHolder> {
	...
	public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
		...
		int splitIndex = Math.min(10, title.length());
		String part1 = title.substring(0, splitIndex);
		String part2 = title.substring(splitIndex);
		...
		Animation animation = AnimationUtils.loadAnimation(context, R.anim.up_from_bottom);
		//holder.binding.startAnimation(animation);
		holder.binding.cardView.startAnimation(animation);
	...
	// Add new videoId + title dynamically with shuffle
	public void addItem(String videoId, String title) {
		videoIds.add(videoId);
		titles.add(title);
		
		// Shuffle ID-title pair together
		ArrayList<Pair> pairList = new ArrayList<>();
		for (int i = 0; i < videoIds.size(); i++) {
			pairList.add(new Pair(videoIds.get(i), titles.get(i)));
		}
